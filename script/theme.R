...
theme_zvis_grid <- function(font_size = 14, line_size = 0.5, rel_small = 12 / 14,
  ...
  cowplot::theme_minimal_grid(
    ...
    rel_small = rel_small, rel_tiny = rel_tiny, rel_large = rel_large,
    colour = colour
  ) %+replace% theme(plot.margin = ggplot2::margin(
    half_line / 2,
    1.5, half_line / 2, 1.5
  ), complete = TRUE)
}
...
theme_zvis_map <- function(font_size = 14, line_size = 0.5, rel_small = 12 / 14,
  ...
  half_line <- font_size / 2
  cowplot::theme_map(
    font_size = font_size, line_size = line_size, rel_small = rel_small,
...
theme_zvis_noy <- function(font_size = 14, line_size = 0.5, rel_small = 12 / 14,
  ...
  cowplot::theme_half_open(
    ...
    theme(
      ...
      legend.position = "top", legend.justification = "right", legend.key.size = unit(
        8,
        "pt"
      ), axis.title.x = element_text(size = 9), complete = TRUE
    )
}
  ...
  cowplot::theme_half_open(
    ...
    theme(
      ...
      legend.position = "top", legend.justification = "right", legend.key.size = unit(
        8,
        "pt"
      ), axis.title.y = element_blank(), plot.title.position = "plot",
